...
export async function POST(request: Request, context: RouteContext<"/api/rooms/[code]/votes">) {
  try {
    enforceRateLimit(request, { key: "rooms:votes", limit: 120, windowMs: 60_000 });
    ...
    const vote = await castVote(code, body?.playerId ?? "", body?.sessionToken ?? "", body?.answerId ?? "", body.vote);

    return Response.json({ vote });
  } catch (error) {
    return jsonError(error instanceof Error ? error.message : "Could not cast vote.", getErrorStatus(error));
  }
}
